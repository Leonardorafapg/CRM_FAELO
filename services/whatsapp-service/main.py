...
app = FastAPI(title="WhatsApp Service", version="0.1.0", lifespan=lifespan)
...
app.add_middleware(
    ...
    allow_headers=["Content-Type", "Authorization", "Accept", "Origin", "X-Request-ID", "X-Webhook-Secret"],
)
...
@app.get("/")
def root():
    return {"status": "online", "service": "whatsapp-service", "version": "0.1.0"}
